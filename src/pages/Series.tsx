...
function SeriesPage(){
    ...
    async function fetchSeries () {
        const res = await fetch(`${BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&language=fr-FR&page=${currentPage}&sort_by=${sort}&with_genres=${genre}`);
    ...
    async function searchSeries () {
        ...
        const res = await fetch(`${BASE_URL}/search/tv?api_key=${TMDB_API_KEY}&language=fr-FR&page=${currentPage}&query=${query}`);
        const data = await res.json();
        ...
        for (let i = 1; i <= total_pages; i++) {
            const res = await fetch(`${BASE_URL}/search/tv?api_key=${TMDB_API_KEY}&language=fr-FR&page=${i}&query=${query}`);
            const data = await res.json();
            unfilteredResults.push(...data.results);
        }
            const filteredResults = unfilteredResults.filter((serie : any) => serie.genre_ids.includes(parseInt(genre)));
            if(sort){
    ...
    return(
        <div className='container'>
            ...
            {(!query || onePage === true) && <div className='sortSelect'>
            ...
            : ''}
            {(series && totalPages >= currentPage && onePage === false) && <Pagination page={page} maxPage={totalPages} />}
        </div>
    )
}
